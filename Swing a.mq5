...
void OnDeinit(const int r){
 ObjectDelete(0,"BtnStart"); ObjectDelete(0,"BtnStop");
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam){
 if(id==CHARTEVENT_OBJECT_CLICK){
  if(sparam=="BtnStart"){ TradingON=true; Print("STARTED"); ObjectSetInteger(0,"BtnStart",OBJPROP_STATE,false); }
  if(sparam=="BtnStop"){ TradingON=false; CloseAll(); ObjectSetInteger(0,"BtnStop",OBJPROP_STATE,false); }
 }
}
bool CloseAll(){
 for(int i=PositionsTotal()-1;i>=0;i--){
  ...
  bool res=OrderSend(rq,rs); if(!res){ rq.type_filling=ORDER_FILLING_FOK; res=OrderSend(rq,rs); }
 }
 return true;
}
void OnTick(){
 ...
 double b=SymbolInfoDouble(_Symbol,SYMBOL_BID); double o=iOpen(_Symbol,PERIOD_H1,0);
 if(f[0]>s[0] && b>o){
  MqlTradeRequest rq; MqlTradeResult rs; ZeroMemory(rq); ZeroMemory(rs);
