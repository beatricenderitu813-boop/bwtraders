...
int OnInit()
{
   ...
   Print("==========================================");
   Print("XAU90 FLIP AI INITIALIZED");
   Print("Symbol: ", _Symbol);
   Print("Timeframe: M1");
   Print("Trigger: ", TriggerPercent, "%");
   Print("Press START to activate the bot.");
   Print("==========================================");
...
void OnDeinit(const int reason)
{
   ...
   Print("XAU90 Flip AI deinitialized. Reason: ", reason);
}
...
void OnTick()
{
   ...
   //--- Calculate live candle strength
...
//+------------------------------------------------------------------+
//| Calculate live M1 directional strength                           |
//+------------------------------------------------------------------+
void CalculateLiveStrength()
{
   ...
   //--- Where is live price inside the forming candle?
...
//| Main reversal logic                                               |
//+------------------------------------------------------------------+
void ProcessFlipLogic()
{
   int NewDirection = 0;

   //===============================================================
   // BUY strength reaches 90%
   // Meaning: current live candle is strongly BUY.
   // Bot takes the opposite side: SELL.
   //===============================================================
   ...
   //===============================================================
   // SELL strength reaches 90%
   // Meaning: current live candle is strongly SELL.
   // Bot takes the opposite side: BUY.
   //===============================================================
   ...
   //--- Already in requested direction
...
//| Reverse position                                                  |
//+------------------------------------------------------------------+
void ReversePosition(int direction)
{
   Print("==========================================");
   ...
   //--- Close existing EA positions first
   CloseOurPositions();

   //--- Immediately open opposite direction
   if(direction == 1)
   {
      Print("ACTION: OPENING BUY");
      OpenBuy();
   }
   else if(direction == -1)
   {
      Print("ACTION: OPENING SELL");
      OpenSell();
   }
...
void OpenBuy()
{
   ...
   if(ask <= 0)
   {
      Print("BUY blocked: invalid ASK price.");
      return;
   }
...
void OpenSell()
{
   ...
   if(bid <= 0)
   {
      Print("SELL blocked: invalid BID price.");
      return;
   }
...
//+------------------------------------------------------------------+
//| Detect existing direction after EA restart                       |
//+------------------------------------------------------------------+
void DetectExistingDirection()
{
   ...
   {
      CurrentDirection = 1;
      Print("Existing EA direction detected: BUY");
   }
   else if(sellCount > buyCount && sellCount > 0)
   {
      CurrentDirection = -1;
      Print("Existing EA direction detected: SELL");
   }
   else
   {
      CurrentDirection = 0;
      Print("No existing EA position detected.");
   }
}
                  ...
                  const string &sparam)
{
   ...
   if(sparam == StartButton)
   {
      ...
      Print("==========================================");
      Print("XAU90 FLIP AI: STARTED");
      Print("LIVE M1 SCANNER: ACTIVE");
      Print("BUY 90% -> SELL");
      Print("SELL 90% -> BUY");
      Print("Waiting for live 90% signal...");
      Print("==========================================");
   ...
   if(sparam == StopButton)
   {
      ...
      Print("==========================================");
      Print("XAU90 FLIP AI: MANUALLY STOPPED");
